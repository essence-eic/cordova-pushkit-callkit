...
- (void)pushRegistry:(PKPushRegistry *)registry didUpdatePushCredentials:(PKPushCredentials *)credentials forType:(NSString *)type{
    ...
    NSLog(@"[objC] PushKit token: %@", sToken);
    for (id callbackId in callbackIds[@"pushRegistry"]) {
       CDVPluginResult* pluginResult = nil;
       pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:sToken];
...
- (void)pushRegistry:(PKPushRegistry *)registry didReceiveIncomingPushWithPayload:(PKPushPayload *)payload forType:(NSString *)type
{
    ...
    callUpdate.localizedCallerName = appName;
    ...
    [self.provider reportNewIncomingCallWithUUID:callUUID update:callUpdate completion:^(NSError * _Nullable error) {
        ...
        NSLog(@"[objC] receivedCall: %@", callUUID);
        
        [NSTimer
          scheduledTimerWithTimeInterval:2.0
          target:self
          selector:@selector(connectCallInside:)
          userInfo:nil
          repeats:NO
        ];
